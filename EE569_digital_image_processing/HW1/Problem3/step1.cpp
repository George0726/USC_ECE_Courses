...

{
	...
	if (argc < 3){
			...
			cout << "program_name input_image.raw output_image.raw [quantize size = 64] [Width = 500] [Height = 400] [BytesPerpixel = 1]" << endl;
			return 0;
	}

    if (argc>3){
        quantize_size = atoi(argv[3]);
        cout<<"change to "<<quantize_size<<" color Version"<<endl;
    }
	if (argc  >5){
		Width = atoi(argv[5]);
	}
    ...
    quantize_image(image_input,image_output,quantize_size,Width,Height,BytesPerPixel);
