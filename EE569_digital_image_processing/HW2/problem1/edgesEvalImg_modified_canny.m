function [thrs,cntR,sumR,cntP,sumP,V,K] = edgesEvalImg_modified_canny( E, G, varargin )
...
Z=zeros(1,n); cntR=Z; sumR=Z; cntP=Z; sumP=Z;
...
%   Z=zeros(size(E)); matchE=Z; matchG=Z; allG=Z;
  for g = 1:n
    [matchE1,matchG1] = correspondPixels(E1,G{g},maxDist);
     ...
     allG = allG + G{g};
     cntR(g) = sum(matchG(:)); sumR(g) = sum(allG(:));
     cntP(g) = nnz(matchE); sumP(g) = nnz(E1);
  end
...
%   if(nargout<6), end; cs=[1 0 0; 0 .7 0; .7 .8 1]; cs=cs-1;
