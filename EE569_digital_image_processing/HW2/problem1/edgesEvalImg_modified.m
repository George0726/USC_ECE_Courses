function [thrs,cntR,sumR,cntP,sumP,V,K] = edgesEvalImg_modified( E, G, varargin )
...
Z=zeros(K,n); cntR=Z; sumR=Z; cntP=Z; sumP=Z;
...
%   Z=zeros(size(E)); matchE=Z; matchG=Z; allG=Z;
  for g = 1:n
    [matchE1,matchG1] = correspondPixels(E1,G{g},maxDist);
     ...
     allG = allG + G{g};
     cntR(k,g) = sum(matchG(:)); sumR(k,g) = sum(allG(:));
     cntP(k,g) = nnz(matchE); sumP(k,g) = nnz(E1);
  end
