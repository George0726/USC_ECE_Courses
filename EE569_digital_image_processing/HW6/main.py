...
parser.add_argument('--data_set',type = str, choices=['mnist','fashion_mnist'],default='fashion_mnist')
parser.add_argument('--is_resume', default='', type=str)
parser.add_argument('--th1',default=0.005,type=float)
...
if __name__ == "__main__":
    ...
    # TODO: fill in this part
    p2 = Pixelhop(depth=3,TH1=args.th1,TH2=0.001,SaabArgs=SaabArgs,shrinkArgs=shrinkArgs,concatArg=concatArg)
    p2.fit(x_train)

    p2.save( "./ph++ckpt/ckpt/dataset_{}_TH1_{}_".format(args.data_set,args.th1))
    ...
    save_feature = "./ph++ckpt/feature/dataset_{}_TH1_{}_.npz".format(args.data_set,args.th1)
    ...
    t_module3 = time.time()
    train_feature_1 = get_feat(x_train[:100,:,:,:],1)

    train_feature_2 = get_feat(x_train[:100,:,:,:],2)

    with open('./ph++ckpt/record.txt', 'a') as f:
        f.write("\ndataset: {};TH1: {};---\n".format(args.data_set,args.th1))
        ...
        f.write("feature size: hop1 {} ; hop2 {}; hop3 {}\n".format(train_feature_1.shape,train_feature_2.shape,train_hop3_feats.shape))
    with open('./ph++ckpt/{}_train_acc.txt'.format(args.data_set), 'a') as f:
        f.write("{}---{}\n".format(args.th1,accuracy_score(y_train, y_train_pred)))
    with open('./ph++ckpt/{}_test_acc.txt'.format(args.data_set), 'a') as f:
        f.write("{}---{}\n".format(args.th1,accuracy_score(y_test, y_test_pred)))
    ...
    np.savez(save_feature,train_feature = train_hop3_feats, train_label = y_train, test_feature = test_hop3_feats, test_label = y_test, train_feature_1 = train_feature_1, train_feature_2 = train_feature_2 )
    print("feature saved!")
